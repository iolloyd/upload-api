module.exports = function ( grunt ) {
  ...
  var taskConfig = {
    ...
    concat: {
      ...
      build_css: {
        src: [
          '<%= vendor_files.css %>',
          '<%= less.options.build_dest %>'
        ],
        dest: '<%= less.options.build_dest %>'
      },
      /**
    ...
    less: {
      options: {
        build_dest: '<%= build_dir %>/assets/<%= pkg.name %>-<%= pkg.version %>.css',
        compile_dest: '<%= compile_dir %>/assets/<%= pkg.name %>-<%= pkg.version %>.css',
      },
      build: {
        options: {
          strictMath: true,
          strictUnits: true,
          sourceMap: true,
          paths: ['vendor/bootstrap/less', 'src/less'],
        },
        files: {
          '<%= less.options.build_dest %>': [ '<%= app_files.less %>' ],
        }
      },
      compile: {
        options: {
        ...
        files: {
          '<%= less.options.compile_dest %>': '<%= less.options.build_dest %>',
        }
      },
    },

    /**
     * `usebanner` prepends the license text.
     */
    usebanner: {
      build_less: {
        options: {
    ...
    jshint: {
      ...
      options: {
        ...
        boss: true,
        //eqnull: true,
        '-W070' : true, // allow trailing comma
      },
      globals: {}
    },
   ...
   * HTTP Server Task
   */
  grunt.registerTask( 'server', 'Start the HTTP Server', function() {
    server();
  });
  ...
  grunt.registerMultiTask( 'index', 'Process index.html template', function () {
    ...
    grunt.file.copy('src/assets/favicon.ico', this.data.dir + '/favicon.ico');
    grunt.file.copy('src/assets/apple-touch-icon-precomposed.png', this.data.dir + '/apple-touch-icon-precomposed.png');
  });
