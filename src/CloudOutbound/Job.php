...
class DemoCombined extends AbstractJob
{
     ...
     * Initializes the job
     */
    protected function initialize(InputInterface $input, OutputInterface $output, array $config)
    {
        $app =$this->getHelper('silex')->getApplication();
        $this->httpSession = new HttpClient($config);
     ...
     * Executes this job
     */
    protected function execute(InputInterface $input, OutputInterface $output, array $cookies)
    {
        ...
        $this->setCookies(array $cookieValues);
        ...
        if ($outbound->getExternalId()) {
            ...
            $output->writeln('<info>Starting outbound upload...</info>');
     ...
     * @return bool
     */
    protected function isLoggedIn(TubesiteUser $tubeuser, $url)
    {
        $response = $this->httpSession->get($url);
    ...
    protected function login(TubesiteUser $tubeuser)
    {
        ...
        $this->logger->info('Login as {user}', ['user' => $tubeuser->getUsername()]);
